...
\subsection{Fenêtre de mention}
Cette fenêtre propose l'édition des mentions du panneau. 
Vous avez comme option :
\begin{itemize}
	\item Le choix du type de police (L4 ou pas, si non elle est déterminée à partir de la couleur de fond du panneau)
	\item Le choix de l'idéogramme. Pour l'instant, vous ne pourrez pas choisir quel idéogramme, mais juste sa présence, ce qui affichera le cadre, vous mettrez le votre manuellement après.
	\item La valeur de la distance à afficher (si la case \og afficher les distances \fg est cochée pour le panneau)
\end{itemize}

\subsection{Fenêtre de cartouche}
Cette fenêtre propose l'édition d'un cartouche.
Vous avez comme option :
\begin{itemize}
	\item Le type (A, N, D, C, R, F ou P), ce qui correspond à la lettre de la classification de la route.
	\item La valeur du numéro de la route. Les annexes ne sont pas encore gérées, vous ne devez mettre que des chiffres.
\end{itemize}

\section{Texte}
Il est possible de générer du texte en indice ou en exposant. Pour cela, vous devez mettre le texte à transformer respectivement entre les caractères \_ (soit \_...\_) et \^ (soit \^...\^).
